      ...
      <div class="left">
        ...
        <zebra-form v-if="!formloading" :option="formOption" :formData="form" @submit="excuteHandle">
          <template slot="formButton">
            <el-upload
              class="upload-demo"
              :action="'/mgr/risk/test/batch/' + rowData.projectNo"
        ...
        <el-row class="example" v-if="!formloading">
          ...
          <p><span></span><span>projectNo</span><span>项目编号</span></p>
          <p style="height:auto;">
            <span>请求体</span>
            <a style="flex:1;">
              <span >[{</span>
...
import {ruleList, singleTest, getTemplate, getResult, getLog} from './api'
...
export default {
  ...
  methods: {
    ...
    excuteHandle (form) {
      ...
      singleTest(this.rowData.projectNo, form).then(res => {
        if(res.data.code == 0 && res.data.data) {
    ...
    downloadModelHandle () {
      ...
      elink.href = '/mgr/risk/test/template/' + this.rowData.id
    ...
    getFormOption () {
      ...
      // 表单配置
      ruleList(this.rowData.id).then(res => {
        if(res.data.code == 0) {
          if(res.data.data){
    ...
    getLogHandle () {
      ...
      this.tableLoading = true
      getLog(this.rowData.projectNo, obj).then(res => {
        if(res.data.code == 0) {
