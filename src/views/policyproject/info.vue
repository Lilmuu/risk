      ...
      <div class="left">
        ...
        <zebra-form v-if="!formloading" :option="formOption" :formData="form" @submit="excuteHandle">
          <template slot="formButton">
            <el-upload
              class="upload-demo"
              :action="'/mgr/risk/test/batch/flow/' + rowData.no"
...
<!--      <viewDetail :modelData="rowData" :rowData="testRow" />-->
      <detail :modelData="rowData" :rowData="testRow" />
    </el-dialog>
  </div>
</template>
<script>
import store from "@/store";
import {ruleList, getTemplate, getResult, getLog} from '../testmanage/api'
import {flowRuleList, singleFlowTest} from './api'
...
// import viewDetail from './testDetail'
import Detail from "./detail";
export default {
  name: 'rule-manage-info',
  components: {Detail, PageHeader, LoadingBox},
  props: {
    rowData: {
      type: Object
    }
  ...
  methods: {
    ...
    excuteHandle (form) {
      ...
      singleFlowTest(this.rowData.no, form).then(res => {
        if(res.data.code == 0 && res.data.data) {
    ...
    downloadModelHandle () {
      ...
      elink.href = '/mgr/risk/test/template/flow/' + this.rowData.no
    ...
    getFormOption () {
      ...
      // 表单配置
      flowRuleList(this.rowData.no).then(res => {
        if(res.data.code == 0) {
          if(res.data.data){
    ...
    viewDetail (row) {
      ...
      this.testRow = data
      // this.testRow = row
      // this.$router.push({ path: 'policyProjectSee', query: JSON.parse(row.modelJson) })
